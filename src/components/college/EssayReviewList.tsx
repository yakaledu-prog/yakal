...
// ============================================================
// A counsellor's view of essays: a queue, and two verbs.
//
// The essay itself lives in Google Docs and is read and commented on there, so
// this is deliberately not an editor. Send it back, or call it finished.
//
// Used twice, and the difference between the two is one prop. On the Essays
// page it spans every student and the rail leads with them. On one student's
// page it is scoped to them, so the rail drops the students and the rows stop
// repeating a name the header already carries. Both were the same screen drawn
// twice before this, and the student's own panel had been standing in for the
// second one: a counsellor looking at a student saw Create doc and Ask for
// review, which are the student's own verbs, and none of their own.
//
...
export interface ReviewListStudent {
  id: string;
...
export function EssayReviewList({
  essays,
  plans,
  catalog,
  counselorId,
  ...
  essays: ReviewQueueItem[];
  plans: Map<string, AdmissionsPlan> | undefined;
  catalog: College[];
  ...
  const [filter, setFilter] = useState<FilterId>(scopedToStudent ? "all" : "waiting");
  ...
  /**
   * The colleges this list has essays for, most work first.
   *
  ...
  const colleges = useMemo(() => {
    ...
    for (const e of pool) {
      const key = e.collegeUnitid
        ? String(e.collegeUnitid)
        : e.kind === "personal_statement"
          ? "personal"
          : "";
      if (!key) continue;
          ...
          <div className="mb-2 flex items-center gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder={
                  scopedToStudent
                    ? "Search a title or a prompt"
                    : "Search a student, a title or a prompt"
                }
          ...
          <div className={cn(compact ? "" : "flex gap-5")}>
            ...
            {!compact && (colleges.length > 1 || (students.length > 1 && !scopedToStudent)) && (
              <nav className="hidden w-[210px] shrink-0 flex-col gap-0.5 md:flex">
                ...
                {students.length > 1 && !scopedToStudent && (
                  <>
  ...
  return (
    ...
    >
      {showStudent ? (
        ...
        // only identity left worth carrying.
        <CollegeLogo
      ...
      <div className="min-w-0 flex-1">
        ...
        {/* Joined rather than concatenated. Dropping the student's name on a
            page already about them used to leave the separator behind it. */}
        <p className="mt-0.5 text-[12.5px] text-muted-foreground">
          {[
            showStudent ? essay.studentName : null,
            essay.collegeName,
            essay.kind === "personal_statement" ? "Personal statement" : null,
            essay.wordLimit != null ? `${essay.wordLimit} words` : null,
          ]
            .filter(Boolean)
            .join(" - ")}
        </p>
