...
export default function Faq({ scrollTo }: { scrollTo: (id: string) => void }) {
  ...
  return (
    <div className="bg-[#f4f4f4] rounded-[20px] md:rounded-[30px] px-[16px] sm:px-[24px] md:px-[73px] py-[60px] md:py-[130px] w-full max-w-[1440px] overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between gap-[4px] md:gap-[80px] w-full min-w-0">
        <Reveal className="w-full md:w-[380px] lg:w-[460px] md:shrink-0 min-w-0 flex flex-col items-center md:items-start text-center md:text-left gap-[4px]">
        ...
        <div className="w-full min-w-0 overflow-hidden md:flex-1 md:max-w-[630px] space-y-[16px] md:space-y-[28px] px-4 md:px-0">
          {faqs.map((faq, idx) => (
            <FaqItem
              key={idx}
              faq={faq}
