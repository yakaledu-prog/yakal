...
const categoryStyles: Record<string, { text: string; rule: string }> = {
  "Study Skills": { text: "text-primary-hover", rule: "bg-tertiary" },
  Admissions: { text: "text-primary", rule: "bg-secondary" },
  "Parent Guide": { text: "text-primary", rule: "bg-primary" },
  "Yakal News": { text: "text-primary-hover", rule: "bg-primary-hover" },
  "Yakal Journal": { text: "text-primary", rule: "bg-primary" },
};
...
function ArticleMeta({ post }: { post: BlogPost }) {
  return (
    <div className="flex flex-wrap items-center gap-x-[9px] gap-y-[4px] text-[12px] text-[#5b6472] md:text-[14px]">
      ...
      <span aria-hidden="true" className="h-[3px] w-[3px] rounded-full bg-secondary" />
    ...
    <section id="blog" aria-labelledby="journal-heading" className="w-full max-w-[1440px] overflow-hidden px-[16px] pb-[44px] sm:px-[20px] md:px-[30px] md:pb-[77px]">
      <Reveal className="mb-[26px] grid gap-[16px] md:mb-[48px] md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
        <div className="max-w-[760px]">
          <p className="mb-[9px] text-[11px] font-semibold uppercase tracking-[0.18em] text-primary sm:text-[13px]">Yakal Journal</p>
        ...
        <button onClick={() => navigate("/posts")} className="group inline-flex min-h-[44px] w-fit items-center gap-[8px] text-[14px] font-semibold text-primary hover:text-primary-hover md:mb-[7px] md:text-[17px]">
          View all articles
      ...
      {featured ? (
        <Reveal>
          <article className="mb-[30px] grid overflow-hidden rounded-[22px] border border-primary/10 bg-[#f5f7f4] shadow-[0_18px_42px_-34px_rgba(16,153,161,0.75)] md:grid-cols-[1.08fr_0.92fr] md:rounded-[30px] md:border-0 md:shadow-none">
            <button
            ...
            <div className="flex flex-col justify-center p-[20px] pb-[22px] sm:p-[28px] md:p-[48px] lg:p-[64px]">
              ...
              <button onClick={() => navigate(`/post/${featured.id}`)} className="group mt-[20px] inline-flex min-h-[44px] w-fit items-center gap-[9px] rounded-[500px] bg-primary px-[22px] py-[11px] text-[14px] font-semibold text-white transition-colors hover:bg-primary-hover md:mt-[36px] md:px-[24px] md:py-[13px] md:text-[15px]">
                Read article
                <ArrowRight aria-hidden="true" size={18} className="transition-transform group-hover:translate-x-1" />
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 gap-[14px] md:grid-cols-3 md:gap-[30px]">
        {blogs.slice(1).map((blog, index) => (
          <Reveal key={blog.id} delay={index * 100}>
            <article className="group grid h-full grid-cols-[104px_minmax(0,1fr)] gap-[14px] border-b border-primary/10 pb-[14px] last:border-b-0 md:flex md:flex-col md:border-b-0 md:pb-0">
              ...
              <div className="flex min-w-0 flex-1 flex-col justify-center md:px-[8px] md:pt-[22px]">
                ...
                <h3 className="mt-[8px] line-clamp-2 text-[18px] font-medium leading-[1.2] tracking-[-0.01em] md:order-2 md:mt-[12px] md:text-[25px] md:leading-[1.3]">
                  <button onClick={() => navigate(`/post/${blog.id}`)} className="min-h-[44px] text-left transition-colors hover:text-primary-hover">{blog.title}</button>
                </h3>
      ...
      <Reveal className="mt-[30px] rounded-[22px] border-t-[4px] border-tertiary bg-[#eef6ef] p-[20px] sm:p-[24px] md:mt-[54px] md:rounded-[30px] md:p-[38px]">
        <div className="grid gap-[18px] md:grid-cols-[minmax(0,1fr)_minmax(360px,0.75fr)] md:items-center md:gap-[40px]">
          <div>
            <p className="mb-[8px] text-[12px] font-semibold uppercase tracking-[0.16em] text-primary-hover">Notes from Yakal</p>
