...
export function CounselorCalendar() {
  ...
  const { data: availabilityInfo } = useQuery({
    queryKey: ['counselor-availability', user?.id],
  ...
  const sessions = useMemo(() => {
    ...
    return rawSessions.map((s: any) => ({
      ...
      counselorName: s.student_name || "Student",
      status: s.status,
    }));
  }, [rawSessions]);
                ...
                {weekDays.map((day, dIndex) => {
                  ...
                  return (
                    <div key={dIndex} className={cn("h-14 border-r border-[#e9edef] dark:border-[#2a3942] last:border-r-0 p-1 flex flex-col gap-1 overflow-hidden", isDisabled && "opacity-40 grayscale bg-[#f0f2f5] dark:bg-[#111b21]/50")}>
                      ...
                      {sessions.map(s => (
                        <div key={s.id} className="bg-primary/5 text-primary border-l-2 border-primary rounded-none px-2 py-1 text-[11px] font-bold truncate cursor-pointer hover:bg-primary/10 transition-colors">
              ...
              <div key={idx} className="grid grid-cols-[100px_1fr] border-b border-[#e9edef] dark:border-[#2a3942] last:border-b-0">
                ...
                <div className="p-2 min-h-[60px] relative">
                  ...
                  <div className="relative z-10 flex flex-col gap-2">
                    {sessions.map(s => (
                      <div key={s.id} className="bg-primary/5 text-primary border-l-2 border-primary rounded-none p-3 text-[13px] hover:bg-primary/10 transition-colors cursor-pointer w-full max-w-md">
  ...
  return (
    <PageWrapper>
      <div className="flex flex-col w-full min-h-screen bg-background dark:bg-[#111b21] pb-12">
        ...
        <div className="bg-primary text-white pt-6 md:pt-10 px-6 md:px-10 pb-6 md:pb-10 relative overflow-hidden shrink-0">
      ...
      <AvailabilityEditor
        ...
        onSave={async (grid, days) => {
          if (user) {
            await saveTutorAvailability(user.id, grid, days);
            queryClient.invalidateQueries({ queryKey: ['counselor-availability', user.id] });
          }
        }}
