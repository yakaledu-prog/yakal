...
import {
  getCounselorSessionsFull, saveSessionNotes, SessionRow,
...
function NotesModal({ session, onClose, onSaved }: { session: SessionRow; onClose: () => void; onSaved: () => void }) {
  ...
  // Notes only. Marking an advising hour complete from here let a counsellor
  // make a month count as delivered for a session that never happened, which is
  // what releases their monthly pay. The hourly job completes it once it has
  // ended, with the attendance in front of it.
  const submit = async () => {
    setSaving(true);
