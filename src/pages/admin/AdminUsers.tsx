              ...
              {filtered.map((u) => (
                <div key={u.id} className="flex items-center gap-3 p-4">
                  ...
                  {(u.role === "tutor" || u.role === "counselor") && u.status === "pending" && (
                    <button onClick={() => act(u, false)} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-[#e9edef] dark:border-[#2a3942] text-[#c0392b] text-[12px] font-semibold hover:bg-[#c0392b]/5">
                      <X size={13} /> Reject
                    </button>
                  )}
                </div>
