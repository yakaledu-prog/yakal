// ============================================================
// Money states, built on demand.
//
// The chain takes days in real time: a lesson has to finish, an earning waits
// 72 hours, a counselling month waits for the month. Testing it honestly would
// be four days per case, so nothing downstream of "the parent paid" ever got
// looked at. This builds those states directly instead.
//
// Lives here rather than in scripts/ because two things call it: the CLI
// (npm run scenarios) and the /dev console, and api/ cannot import from
// scripts/ since scripts are not deployed.
//
// Local stack only, and deliberately not overridable. It writes invoices,
// refunds and settled earnings, which are claims that money moved, and it
// creates real Stripe test-mode charges. None of that belongs in a real ledger.
// ============================================================

import Stripe from 'stripe';
import { createClient } from '@supabase/supabase-js';

/** Where output goes. The CLI prints it; the dev console collects it. */
...
if (!/localhost|127\.0\.0\.1/.test(URL_LOCAL)) {
  // Thrown rather than exited: this is imported by the API too, and a module
  // that kills the process takes the server with it.
  throw new Error('Scenarios only ever run against the local stack.');
}
...
async function realCharge(amountCents: number, description: string, log: Log): Promise<string | null> {
  if (!stripe) {
    if (!warnedNoStripe) {
      log('  (no STRIPE_SECRET_KEY, so charges are not created and refunds cannot be tested)');
      warnedNoStripe = true;
    }
    return null;
  }
  try {
    ...
    log(`  (Stripe refused a charge for ${description}: ${err?.message})`);
    return null;
  }
}
...
async function makeInvoice(c: Cast, label: string, amount: number, share: number, log: Log, status = 'paid') {
  const chargeId = status === 'paid' ? await realCharge(amount, label, log) : null;
...
export async function build(log: Log = console.log) {
  warnedNoStripe = false;
  ...
  const inv1 = await makeInvoice(c, 'upcoming lesson', coursePrice, tutorShare, log);
  ...
  const inv3 = await makeInvoice(c, 'due lesson', coursePrice, tutorShare, log);
  ...
  log(`  settled by ACH      ${usd(tutorShare)}  <- invisible to Stripe's 1099 draft`);

  // Cancelled in notice: everything back, tutor earns nothing.
  const inv6 = await makeInvoice(c, 'cancelled in notice', coursePrice, tutorShare, log);
  ...
  log(`  refunded in full    ${usd(coursePrice)} back, tutor earns nothing`);

  // Cancelled late: half kept, and the tutor keeps half of their share.
  const inv7 = await makeInvoice(c, 'cancelled late', coursePrice, tutorShare, log);
  ...
  log(`  refunded half       ${usd(lateRefund)} back, tutor keeps ${usd(lateShare)}`);

  // Billed, and nobody came.
  const inv8 = await makeInvoice(c, 'no show', coursePrice, tutorShare, log);
  await makeSession(c, 'no show', inv8.id, -days(2), 'no-show', tutorShare, {
    attendance: [], attendance_checked_at: iso(-days(2)),
  });
  log(`  no-show             billed, nobody joined, nothing paid`);

  // An invoice nobody ever paid.
  await makeInvoice(c, 'abandoned checkout', coursePrice, tutorShare, log, 'open');
  log(`  unpaid              ${usd(coursePrice)} open`);
  ...
  await makeEarning({
    ...
    source_charge_id: await realCharge(tierPrice, 'plan running', log),
  });
  ...
  await makeEarning({
    ...
    source_charge_id: await realCharge(tierPrice, 'plan delivered', log),
  });
  log(`  plan delivered      ${usd(tierShare)} payable, an advising hour happened`);
  ...
  await makeEarning({
    ...
    period_start: dateOf(-days(35)), period_end: iso(-days(5)),
    amount_cents: tierShare, status: 'pending', releasable_at: iso(-days(2)),
    source_charge_id: await realCharge(tierPrice, 'plan idle', log),
  });
  log(`  plan idle           ${usd(tierShare)} refused, nothing was delivered`);
...
async function runJob(): Promise<any> {
  ...
  const handler = (await import('../_handlers/run-jobs.js')).default;
  const out: any = {};
  const res: any = {
 ...
 */
export async function fastForward(log: Log = console.log) {
...
export async function release(log: Log = console.log) {
...
export async function status(log: Log = console.log) {
  ...
  log('\nEarnings ledger\n');
  log(`  held (hold running)   ${usd(sum((r) => r.status === 'pending' && new Date(r.releasable_at).getTime() > now))}`);
  log(`  owed (hold expired)   ${usd(sum((r) => r.status === 'pending' && new Date(r.releasable_at).getTime() <= now))}`);
  log(`  settled through Stripe ${usd(sum((r) => r.status === 'settled' && r.method === 'stripe_connect'))}`);
  log(`  settled another way    ${usd(sum((r) => r.status === 'settled' && r.method !== 'stripe_connect'))}`);
  log(`  cancelled             ${usd(sum((r) => r.status === 'cancelled'))}`);
  ...
  log(`Invoices   ${Object.entries(invByStatus).map(([k, v]) => `${k} ${v}`).join(', ') || 'none'}`);

  const { data: refunds } = await db.from('refunds').select('amount_cents, reason');
  log(`Refunds    ${(refunds ?? []).length}, ${usd((refunds ?? []).reduce((n, r: any) => n + r.amount_cents, 0))}`);
  log('');
}

export async function clear(log: Log = console.log) {
