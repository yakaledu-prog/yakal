...
course, because that is the one moment somebody has to pick from _their_ own  
classes.
...
Take the deployed one from **Vercel > Settings > Domains** and add every
address people actually open, custom domain included. Google compares the
origin as a string, so all three of these are different from what you meant:

- a trailing slash, `https://example.com/`
- any path, `https://example.com/admin/courses`
- the wrong scheme, `http` where the site is served over `https`

Getting it wrong answers `Error 400: origin_mismatch` on the sign-in popup.
Allow a few minutes after saving, then reload hard.

**Wildcards are not supported here.** Vercel gives every preview deployment
its own hostname, so Fetch Details will fail on previews whatever you
register. Either add the stable branch alias for the branch you preview from,
or link classes on production only. Nothing else is affected: Drive and the
tutor and student assignment lists go through the server and never involve the
browser's origin.
...
may work, because that is signed in as _you_, while the tutor and student  
...
**Admin > Courses > a linked course > Assignments.**  
...
**Tutor > My Courses > the same course > Assignments.** Should match step 2  
exactly, with no Google sign-in anywhere.

**Student > My Learning > the course > Assignments.** Same again. A student  
is never asked to sign in to Google; if they are, something is calling  
Google from the browser that should not be.

**Student > College > Documents.** Uploads and lists through Drive on the  
same credential.
