...
psql(`insert into course_tutors (course_id, tutor_id) select '${courseId}', id from profiles where email='tutor@yakal.com' on conflict do nothing;`);
...
let posted = null;
await p.route('**/api/stripe?action=create-invoice', async (route) => {
  posted = route.request().postDataJSON();
  await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ invoiceId: 'test' }) });
});
await p.route('**/api/stripe?action=checkout', (route) =>
...
// Case-insensitive: the course sits above the tutor's name as an uppercased eyebrow.
pass('the real course is shown', /Chemistry, Grade 11 Foundations/i.test(text));
pass('the tutor is named alongside it', /Bethlehem Alemu/.test(text));
...
pass('it says who it is for', /Amen Worku/.test(text));
...
const nextWeek = p.getByRole('button', { name: 'Next week' });
await nextWeek.waitFor({ timeout: 15000 });
...
// The grid starts at 08:00, which is HOUR_START on the page and the first row
// of the tutor's stored availability. An off-by-one here books every session
// an hour out, which is the bug this pins: the grid used to draw 15 rows from
// 7 AM against a 13-row 8 AM grid. This line still asserted the old 07:00, so
// it was red against the fix rather than guarding it.
pass('the slot time matches the one clicked', posted?.booking?.[0]?.startTime === '08:00',
...
// A course with an empty roster cannot be booked.
psql(`delete from course_tutors where course_id='${courseId}';`);
await p.goto(`${BASE}/parent/courses/${courseId}`, { waitUntil: 'domcontentloaded' });
